...
S32 CATH_INSTRUCTION_GET_DISP(const SH_INSTRUCTION* INSTR)
{
    ...
    switch(INSTR->INSTR_ID)
    {
        ...
        case CATH_INSTR_ID_BSR:
        {
            ...
            DISP = (DISP12 & SH2_INSTR_DISP_SIGN_BIT) ? (S32)(DISP12 | SH2_INSTR_DISP_SIGN_MASK) : (S32)DISP12;
            break;
        }
    ...
    // NOW WE CAN DYNAMICALLY GET THE SCALE FROM YOUR TABLE
...
{
    UNK TOTAL_SIZE = 0;
    const char* SH2_OPCODE = CATH_GET_OPCODE_NAME(INSTR->INSTR_ID);

    // COPY THE CURRENT OPCODE INTO THE BUFFER BASED ON THE 
    // SIZE OF IT'S MASK
    CATH_BUFFER_CPY(DEST, TOTAL_SIZE, SH2_OPCODE);
