...
//
// 30/06/26
// CHANGES HAVE BEEN MADE FOLLOWING THE REVEALS AND EDGE CASES
// THROUGH TESTING
// 
// WHAT USED TO HAPPEN WITH THE SIZE ACCESS
// FROM THE DISASSMEBLED INSTRUCTION IS THAT IT WOULD CAUSE TRUNCATION
// OF THE OPERANDS STRING LITERAL
//
// NOW, THE FOLLOWING WILL MAKE SURE TO:
// - LOOK FOR A VALID INSTRUCTION
// - DETERMINE THE AMOUNT OF OPERANDS AND THEIR TYPES
// AND PROPERLY ACCOUNT FOR THAT SIZING WITHIN THE BUFFER
// TO AVOID TRUNCATION

UNK CATH_INSTRUCTION_GET_SIZE(const SH_INSTRUCTION* INSTR)
{
    // 09/07/26
    // THE INITIAL ASSUMPTION WAS THAT A FULLY PROCESSED INSTRUCTION
    // WOULD BE PASSED THROUGH WITH A NON-NULL DESCRIPTOR 
    //
    // LEVERAGE AN EXPLICIT CHECK TO MAKE SURE THAT PROCESS DOES IT'S JOB
    ...
    {
        TOTAL_SIZE += 1;
        TOTAL_SIZE += CATH_INSTRUCTION_DISASM_OPERAND(INSTR, NULL);
    }
