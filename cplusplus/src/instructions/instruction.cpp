...
{
    UNK SIZE = CATH_INSTRUCTION_DISASM_OPERAND(&this->INSTR, nullptr, 0);
    std::string RESULT(SIZE, '\0');

    CATH_INSTRUCTION_DISASM_OPERAND(&this->INSTR, RESULT.data(), SIZE + 1);
    return RESULT;
}
...
std::string CATH_INSTRUCTION::CATH_DISASSEMBLE_INSTR() const
{
    ...
    CATH_INSTRUCTION_DISASM_INSTR(&this->INSTR, RESULT.data(), SIZE + 1);
    return RESULT;
}
...
std::string CATH_INSTRUCTION::CATH_DISASSEMBLE() const
{
    ...
    CATH_INSTRUCTION_DISASM(&this->INSTR, RESULT.data(), SIZE + 1);
    return RESULT;
}
