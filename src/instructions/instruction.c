...
UNK CATH_INSTRUCTION_DISASM_DATA(const SH_INSTRUCTION* INSTR, char* DEST, UNK SIZE)
{
    ...
    return snprintf(DEST, SIZE, "0x%04x ", CATH_GET_RAW(INSTR));
}

// NOW WE DETERMINE THE SIZE OF THE CURRENT INSUTRCTION
UNK CATH_INSTRUCTION_GET_SIZE(const SH_INSTRUCTION* INSTR)
{
    UNK DATA_SIZE  = CATH_INSTRUCTION_DISASM_DATA(INSTR, NULL, 0);
    UNK INSTR_SIZE = CATH_INSTRUCTION_DISASM_INSTR(INSTR, NULL, 0);
    return DATA_SIZE + INSTR_SIZE;
}
...
UNK CATH_INSTRUCTION_DISASM_INSTR(const SH_INSTRUCTION* INSTR, char* DEST, UNK SIZE)
{
    ...
    if(DEST != NULL)
    {
        UNK COPIED = (INSTR_LENGTH < SIZE) ? INSTR_LENGTH : (SIZE > 0 ? SIZE - 1 : 0);
...
UNK CATH_INSTRUCTION_DISASM(const SH_INSTRUCTION* INSTR, char* DEST, UNK SIZE)
{
    ...
    if(SH2_INSTR_VALID(INSTR))
    {
        ...
        UNK DATA = CATH_INSTRUCTION_DISASM_DATA(INSTR, BUFFER_WRITE, SIZE);
