...
static CATH_INSTR_ID CATH_SH2_ID_TABLE[0x10000];
static bool CATH_SH2_ID_TABLE_INIT = false;

// DEFINE THE BASIS BY WHICH WE ARE ABLE TO LOOKUP ENTRIES WITHIN THE TABLE
// FROM THERE, GENERATE THE LOOKUP TABLE BASED ON THE ABOVE CORRESSPONDENCE
static const SH_ENTRY CATH_SH2_DECODE_TABLE[CATH_INSTR_ID_ALL_MAX] =
{
    ...
    { 0xF0FF, 0x0023, CATH_INSTR_ID_BRAF },
    ...
    { 0xF0FF, 0x4002, CATH_INSTR_ID_STSMACHPD },
    { 0xF0FF, 0x4012, CATH_INSTR_ID_STSMACLPD },
    { 0xF0FF, 0x4022, CATH_INSTR_ID_STSPRPD },
    { 0xF0FF, 0x4013, CATH_INSTR_ID_STCGBRPD },
    ...
    { 0xF0FF, 0x4006, CATH_INSTR_ID_LDSMACHPI },
    { 0xF0FF, 0x4016, CATH_INSTR_ID_LDSMACLPI },
    { 0xF0FF, 0x402A, CATH_INSTR_ID_LDSPR },
    { 0xF0FF, 0x4026, CATH_INSTR_ID_LDSPRPI },
    { 0xF0FF, 0x400F, CATH_INSTR_ID_MACW },
    ...
    { 0xFF00, 0x8000, CATH_INSTR_ID_MOVBS4 },
    { 0xFF00, 0x8100, CATH_INSTR_ID_MOVWS4 },
    ...
    { 0xFFFF, 0x0000, CATH_INSTR_ID_INVALID }
};
...
//
// WHAT MAKES THIS IMPLEMENTATION DIFFERENT IS THAT 
// RATHER THAN EVALUATING AGAINST ENUMERATIVE ENTRIES
// WHICH DON'T ACCESS THE FULL RANGE, WE CAN INSTEAD
// PRE-COMPUTE THE RESULT TO A MAX ELEMENT 
// AMD MASK-MATCH ALL THE SAME

void CATH_INSTRUCTION_BUILD_ID_TABLE(void)
{
    // DETERMINE IF THE TABLE HAS ALREADY BEEN PRE-COMPUTED
    // NEGATES ANY RUNTIME INFERENCE
    if(CATH_SH2_ID_TABLE_INIT) return;

    for(U32 INDEX = 0; INDEX < 0x10000; INDEX++)
    {
        CATH_SH2_ID_TABLE[INDEX] = CATH_INSTR_ID_INVALID;

        for(const SH_ENTRY* ENTRY = CATH_SH2_DECODE_TABLE; ENTRY->MASK != 0; ENTRY++)
        {
            if(((U16)INDEX & ENTRY->MASK) == ENTRY->PATTERN)
            {
                CATH_SH2_ID_TABLE[INDEX] = ENTRY->ID;
                break;
            }
        }
    }

    CATH_SH2_ID_TABLE_INIT = true;
}

// APPLIES THE RELEVANT ENTRY ID IN RELATION TO IT'S RESPECTIVE ENUM VALUE
// BASED ON MASK-MATCH EVALUATION
static CATH_INSTR_ID CATH_INSTRUCTION_PROCESS_UNIQUE_ID(const SH_INSTRUCTION* INSTR)
{
    return CATH_SH2_ID_TABLE[INSTR->WORD];
}
...
void CATH_INSTRUCTION_PROCESS(SH_INSTRUCTION* INSTR)
{
    ...
    // 09/07/26
    // NEGATE THE NULL DEREFERENCE ENTIRELY BY DOWNSTREAMING THE CHECKS HERE
    // ASSUMES IF THE FOLLOWING INSTRUCTION HAS A VALID DESCRIPTOR ENTRY
    // AS OTHER FUNCTIONS RELY ON SUCH AN IMPLEMENTATION REGARDLESS

    assert(INSTR->DESCRIPTOR != NULL);
    return;
}
